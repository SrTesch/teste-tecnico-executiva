...
export class TasksController {
  ...
  public getAll = async (req: Request, res: Response): Promise<Response> => {
    ...
    const userId = req.user.id; // Pego do middleware de autenticação!
  ...
  public create = async (req: Request, res: Response): Promise<Response> => {
    ...
    const task = await this.tasksService.create({ title, description, user_id: userId, status: 'pendente' });
  ...
  public delete = async (req: Request, res: Response): Promise<Response> => {
    ...
    // 204 No Content é uma resposta comum para delete bem-sucedido
